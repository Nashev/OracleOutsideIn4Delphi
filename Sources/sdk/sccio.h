...
typedef struct IOSPECCABFILEtag
{
    HIOFILE                 hRefFile;   /* cab file (block I/O) */

    VTWORD                  wFlags;         /* see values below */
    VTWORD                  wFileIndex;     /* file index */
    VTWORD                  wszFileName[512];/* null terminated file name */
} IOSPECCABFILE, * PIOSPECCABFILE;
/* possible values for IOSPECCABNODE.dwFlags */
#define IO_CABFLAG_INDEX    0x0000
#define IO_CABFLAG_NAME     0x0001
...
#ifdef SCCFEATURE_64BITIO
    VTOFF_T                 dwLoc;      /* offset of actual file header */
    VTOFF_T                 dwDataLen;  /* length of compressed data */
#else
    VTDWORD                 dwLoc;      /* offset of actual file header */
    VTDWORD                 dwDataLen;  /* length of compressed data */
#endif
...
typedef struct IOZIPFILEtag
{
    ...
    VTDWORD                 dwFileCnt;  /* number of files in zip */
...
typedef struct IOZIPNODEtag
{
    ...
    VTDWORD                 dwIndex;    /* index into IOZIPFILE.pFileTable */
...
/*********************************************************************/
/*********************************************************************/
#define FLATE_WINDOW                32768   /* buffer size */
#define FLATE_MASK                  (FLATE_WINDOW - 1)
#define FLATE_MAX_HUFFMAN           15      /* max Huffman code length */
#define FLATE_MAX_CODE_LEN_CODES    19      /* max number of code length codes */
#define FLATE_MAX_LIT_CODES         288     /* max number of literal codes */
#define FLATE_MAX_DIST_CODES        30      /* max number of distance codes */
#define FLATE_MAX_LENGTH            260     /* max number of length for  <length, backward distance> pair */
                                            /* (it is max 258 bytes according to RFC 1951) */

/* Structure to store input for creating the Huffman tree (Huffman code table entry) */
typedef struct FLATECODEtag
{
    VTSHORT nLength;    /* produced code lengths    */
    VTLONG  lCode;      /* produced codes           */
    VTLONG  lValue;     /* TODO:??                  */
} FLATECODE, * PFLATECODE;

/* Huffman code table */
typedef struct FLATEHUFFMANTABLEtag
{
    VTHANDLE        hStart; /* handle  to indexes of the first code of each length [IO_FLATE_MAX_HUFFMAN+2] */
    VTLONG*         pStart; /* pointer to indexes of the first code of each length [IO_FLATE_MAX_HUFFMAN+2] */
    PFLATECODE      pFlateCodes;
} FLATEHUFFMANTABLE, * PFLATEHUFFMANTABLE;

/* Decoding info for length and distance code words */
typedef struct FLATEDECODEtag
{
    VTLONG lBits;   /* number of extra bits */
    VTLONG lFirst;  /* first length/distance */
} FLATEDECODE, * PFLATEDECODE;

typedef struct IOFLATEDATAtag
{
    FLATEHUFFMANTABLE sLiteralCodeTab;  /* literal code table */
    FLATEHUFFMANTABLE sDistanceCodeTab; /* distance code table */
    VTHANDLE    hAllCodes;              /* handle to literal and distance codes. */
    PFLATECODE  pAllCodes;              /* pointer to literal and distance codes [IO_FLATE_MAX_LIT_CODES + IO_FLATE_MAX_DIST_CODES]. */
    VTLONG      lCodeBuf;               /* input buffer */
    VTLONG      lCodeSize;              /* number of bits in input buffer */
    VTBOOL      bCompressedBlock;       /* set if reading a compressed block */
    VTDWORD     dwBlockLen;             /* remaining length of uncompressed block */
    VTBOOL      bEndOfBlock;            /* set when end of block is reached */
    VTBOOL      bEOD;                   /* set when end of stream is reached */
    VTHANDLE    hDecompData;            /* handle to decompressed data buffer extracted form  <length, backward distance> pair. */
    VTBYTE * pDecompData;               /* buffer which will keep extracted decompressed data */
    VTDWORD     dwRemain;               /* number valid bytes in output buffer */
    /* predictor support */
    VTWORD       wSlidingWindowSize;    /* calculated from CINFO flag, and used to allocate buffer for sliding window */
    VTHANDLE  hPredictorData;           /* handle to the predictor sturcture. */
    VTBYTE * pPredictorData;            /* pointer to predictor sturcture [FLATE_MAX_LENGTH]. */
} IOFLATEDATA, * PIOFLATEDATA;
